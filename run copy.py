...
def train_epoch(model, training_data, hypergraph_list, id2vector, optimizer):
    # train
    model.train()
    total_loss = 0.0
    ...
    for i, batch in enumerate(training_data):
        ...
        # my_dict = {'a': 1, 'b': 2, 'c': 3}
        vector_list = [value for value in id2vector.values()]
        
        
        pred= model(tgt, hypergraph_list, vector_list[0])
...
def train_model(HGSL, data_name):
    ...
    id2Vector = []
    # !@添加数据
    with open('data/gos/id2Vector.pickle', 'rb') as file:
        id2Vector = pickle.load(file)
        # print(id2Vector)
    
    opt.textVector_size = len(id2Vector)


    # print(hypergraph_list[1][1].size())
    # print(user_size)
    ...
    # print(id2Vector[3341])
    
    opt.user_size = user_size
    opt.edge_size = news_size+1
    opt.lenvec = len(id2Vector)
    # print(opt.lenvec)
    ...
    for epoch_i in range(opt.epoch):
        ...
        train_loss = train_epoch(model, train_data, hypergraph_list, id2Vector, optimizer)
        ...
        if epoch_i > 1:
            #start = time.time()
            scores = test_epoch(model, valid_data, hypergraph_list, id2Vector)
            print('  - (Validation) ')
            for metric in scores.keys():
                print(metric + ': ' + "%.5f"%(scores[metric]*100) +"%")

            print('  - (Test) ')
            scores = test_epoch(model, test_data, hypergraph_list, id2Vector)
            for metric in scores.keys():
                print(metric + ': ' + "%.5f"%(scores[metric]*100) +"%")
...
def test_epoch(model, validation_data, hypergraph_list, id2vector):
    ...
    with torch.no_grad():
        for i, batch in enumerate(validation_data):
            ...
            # forward
            pred = model(tgt, hypergraph_list, id2vector)
